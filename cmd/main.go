...
import (
	...
	"github.com/flokiorg/grpc-miner/network"
...
func main() {
	...
	flogger := utils.CreateFileLogger(filepath.Join(logDir, "fminer.log"))
	cRequest := &pb.CandidateRequest{
		MiningAddrs:    cfg.MiningAddrs,
		Xpub:           cfg.Xpub,
		CoinbaseScript: cbs,
	}

	miner := mining.NewMiner(a, cRequest, cfg.PoolServer, cfg.SlowDownDuration, cfg.Threads, flogger, cfg.MineOnce)
...
func parseCoinbaseScript(input string) (int, string, int, error) {
	...
	leftBytes, err := utils.ParseIntWithDefault(parts[0], network.DefaultCBSBoundaryBytesSize)
	if err != nil {
	...
	rightBytes, err := utils.ParseIntWithDefault(parts[2], network.DefaultCBSBoundaryBytesSize)
	if err != nil {
		return 0, "", 0, fmt.Errorf("invalid right bytes: %v", err)
	}

	totalLength := leftBytes + len(customText) + rightBytes
	if totalLength > network.MaxCoinbaseScriptSize {
		return 0, "", 0, fmt.Errorf("total byte length %d exceeds maximum allowed %d", totalLength, network.MaxCoinbaseScriptSize)
	}
